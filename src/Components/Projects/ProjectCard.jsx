  ...
  const handleMoreSkills=()=>{
    ...
    return (
        <div id={props.key} className="xl:w-1/4 md:w-1/2 p-4">
                    <div className={`bg-gray-100 p-4 rounded-lg  overflow-hidden ${toggleSkill?"h-fit":"h-[28rem]"}`}>
                      ...
                      <h2 className="h-14 text-lg  font-bold ml-1 text-gray-800 text-center   mb-1">{props.title}</h2>
                      <p className="leading-relaxed text-base text-ellipsis ml-1 line-clamp-4 text-left">{props.overview}</p>
                      <div className={`mt-5 py-1 flex flex-wrap justify-center gap-y-5 gap-x-2 overflow-hidden ${toggleSkill?"h-fit":"h-8"}`}>
                        {
                          skills.map((skill,index)=>(
                            <div key={index} className='flex justify-center items-center h-fit w-fit  bg-slate-800 text-gray-100 py-1 px-4 text-nowrap  rounded-full text-xs font-serif '>{skill}</div>
                          ))
                        }                        
                      </div>
